    ...
    return (
        <div className="checkout-list-item">
            <div className="checkout-item-product">
               <div className="checkout-item-product-image" style={{backgroundImage:`url(${coverPhoto})`, backgroundSize:"100% 100% ,cover"}}></div>
               <Paragraph fontSize={18} fontWeight="semi-bold">{title}</Paragraph>
            </div>
            <div className="checkout-item-quantity flex" style={{flexFlow:"row"}}>
                <Button onClick={()=> addProductToCart(product)}  fontWeight="bold"  color="black" background="white" style={{borderTopLeftRadius:"20px", borderBottomLeftRadius:"20px", transform:"translateX(6px)"}}>+</Button>
                <Button fontWeight="bold" color="black" background="white" style={{transform:"translateY(0px)"}}>{quantity}</Button>
                <Button onClick={() => removeItemFromCart(id)} fontWeight="bold" color="black" background="white" style={{borderTopRightRadius:"20px", borderBottomRightRadius:"20px", transform:"translateX(-6px)"}} >-</Button>
            </div>
            <div className="checkout-item-price flex" style={{flexFlow:"row"}}>
                <Paragraph fontSize={20} fontWeight="semi-bold">{cost}Rs</Paragraph>
            </div>
            <div className="checkout-item-cross flex" style={{flexFlow:"row"}}>
            <Button onClick={() => deleteAllProductsFromCart(id)} fontWeight="bold" fontSize={18}  color="black" background="none" style={{transform:"translateY(0px)"}}>X</Button>
            </div>
            {/* <br/>
